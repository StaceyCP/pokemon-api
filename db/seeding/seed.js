...
const seed = ({
  ...
  return db
    ...
    .then(() => {
      const generationsInsertStr = format(
        `INSERT INTO generations (generation, new_pokemon, total_pokemon) 
            VALUES %L RETURNING *;`,
        generations.map(({ generation, new_pokemon, total_pokemon }) => {
          return [generation, new_pokemon, total_pokemon];
        })
      );
      return db.query(generationsInsertStr);
    })
    .then(() => {
      const typesInsertStr = format(
        `INSERT INTO types (type, generation, description)
            VALUES %L RETURNING *;`,
        types.map(({ type, generation, description }) => {
    ...
    .then(() => {
      const pokemonInsertStr = format(
        `INSERT INTO pokemon (name, spriteURL, generation)
            VALUES %L RETURNING *;`,
        pokemon.map(({ name, spriteURL, generation }) => {
    ...
    .then(() => {
      const pokemon_typesInsertStr = format(
        `INSERT INTO pokemon_types (name, type)
            VALUES %L RETURNING *;`,
    ...
    .then(() => {
      const abilitiesInsertstr = format(
        `INSERT INTO abilities (name, description, generation)
            VALUES %L RETURNING *;`,
        abilities.map(({ name, description, generation }) => {
    ...
    .then(() => {
      const pokemon_abilitiesInsertStr = format(
        `INSERT INTO pokemon_abilities (name, ability, hidden)
            VALUES %L RETURNING *;`,
        pokemonAbilities.map(({ name, ability, hidden }) => {
